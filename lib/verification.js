...
// Build the light, "AI-shaped" assessment for an ID-only (/verify) submission
// from a Stripe Identity result, so those rows render a verdict + pros/cons on
// the dashboard without a real Claude photo assessment. Shared by /api/verify
// (at submit time) and /api/submissions (the "processing" re-check), so a check
// that was still processing when the renter finished flips to the SAME verified
// result once Stripe resolves it — no drift between the two code paths.
export function buildIdOnlyAssessment(applicantName, stx) {
  const s = stx || {};
  ...
  return { recommendation, ai, verified };
}

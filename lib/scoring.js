...
function scoreInsurance(d) {
  ...
  } else if (d.hasIns === "yes") {
    flags.push({ level: "good", text: "Has active auto insurance." }); lines.push("Has insurance");
  }
...
function scoreRental(d) {
  ...
  // The form no longer collects incident counts, so a returning renter with no
  // data scores neutral — do not describe an unanswered history as "positive".
  const anyDetail = d.lateReturns || d.damageClaims || d.smoking || d.chargebacks || d.commIssues;
  if (!anyDetail) {
    lines.push("Has rented before — incident details not collected");
    return { points: 0, cap: CONFIG.caps.rental, flags, lines };
  }

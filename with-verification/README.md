# Josh Rental — Renter Vetting System  ·  VERIFICATION EDITION

> **This is the "with verification" version.** The identity step uses an interactive
> **Stripe Identity** flow and the insurance step uses an interactive **Canopy Connect**
> flow, and the dashboard shows authoritative "✓ Verified" results (including failures).
>
> ⚠️ **These verification flows are a fully-interactive MOCK** for demonstrating the
> experience — they are not wired to the real Stripe/Canopy APIs yet. To make them
> real, you'd add each provider's SDK + webhook (accounts/keys required). Everything
> else (form, scoring, AI, dashboard, decisions) is identical to the standard version.
>
> The plain version (no verification) is in the **"Josh Rental Vetting"** folder.
